...
const CAT_CONFIG = {
  "胸":     { color:"#e53935", bg:"#fff0f0", light:"#ffcdd2", icon:"ti-heart-rate-monitor" },
  "背中":   { color:"#1565c0", bg:"#f0f4ff", light:"#bbdefb", icon:"ti-arrow-back-up" },
  "肩":     { color:"#f9a825", bg:"#fffde7", light:"#fff9c4", icon:"ti-arrows-left-right" },
  "三頭筋": { color:"#6a1b9a", bg:"#f9f0ff", light:"#e1bee7", icon:"ti-arm" },
  "二頭筋": { color:"#00838f", bg:"#e0f7fa", light:"#b2ebf2", icon:"ti-arm" },
  "足":     { color:"#2e7d32", bg:"#f0fff4", light:"#c8e6c9", icon:"ti-run" },
  "有酸素": { color:"#e65100", bg:"#fff3e0", light:"#ffe0b2", icon:"ti-walk" },
  "その他": { color:"#546e7a", bg:"#f5f7f8", light:"#cfd8dc", icon:"ti-dots" },
};
...
const calcBurnKcal = (cat, sets, reps, weight) => {
  ...
  return Math.round(met * 60 * minutes / 60);
};
...
const gs = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Noto Sans JP',sans-serif;background:#f7f7f7}
  input,select{font-family:inherit;outline:none}
  input[type=number]::-webkit-inner-spin-button{-webkit-appearance:none}
  .inp{width:100%;padding:11px 13px;border-radius:10px;border:1.5px solid #ddd;background:#fff;font-size:15px;font-weight:700;color:#111}
  .inp:focus{border-color:#111}
  .inp::placeholder{color:#bbb;font-weight:400}
  .sel{width:100%;padding:11px 13px;border-radius:10px;border:1.5px solid #ddd;background:#fff;font-size:15px;font-weight:700;color:#111;cursor:pointer}
  .sel:focus{border-color:#111}
  .btn-black{width:100%;padding:13px;border-radius:10px;border:none;background:#111;color:#fff;font-size:15px;font-weight:900;cursor:pointer}
...
export default function App() {
  ...
  const addMeal = () => {
    ...
    setMeals(p => ({ ...p, [td]: [ ...(p[td] || []), { id: Date.now(), type: 'food', food: selectedFood, grams: parseFloat(grams), kcal } ] }));
    setFoodQuery(''); setSelectedFood(''); setGrams(''); setFoodSugg([]);
  };

  const removeMeal = (id) => setMeals(p => ({ ...p, [td]: (p[td] || []).filter(m => m.id !== id) }));

  const addWorkout = (ex = exercise, s = sets, r = reps, w = weight) => {
    if (!ex || !trainingCat) return;
    const burnKcal = calcBurnKcal(trainingCat, parseInt(s) || 1, parseInt(r) || 0, Number(w));
    setWorkouts(p => ({ ...p, [td]: [ ...(p[td] || []), { id: Date.now(), cat: trainingCat, exercise: ex, reps: parseInt(r) || 0, weight: Number(w), sets: parseInt(s) || 1, burnKcal } ] }));
    setMeals(p => ({ ...p, [td]: [ ...(p[td] || []), { id: Date.now() + 1, type: 'burn', food: `${trainingCat}(${ex})`, kcal: burnKcal } ] }));
    setExercise(''); setReps(''); setSets(''); setWeight(0);
    setShowHistory(false);
  };

  const addFromHistory = (w) => {
    setExercise(w.exercise); setSets(String(w.sets)); setReps(String(w.reps)); setWeight(w.weight); setShowHistory(false);
  };

  const removeWorkout = (id) => setWorkouts(p => ({ ...p, [td]: (p[td] || []).filter(x => x.id !== id) }));
      ...
      <div style={page}>
        ...
        <div onClick={() => setScreen('training')} style={{ ...card, borderLeft: '5px solid #111', cursor: 'pointer', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
            ...
            <i className="ti ti-chevron-right" style={{ color: '#ccc', fontSize: 20 }}></i>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {CATEGORIES.map(c => {
              const cnt = (workouts[td] || []).filter(w => w.cat === c).length;
              const cfg = CAT_CONFIG[c];
              return cnt > 0 ? <span key={c} style={{ fontSize: 12, fontWeight: 700, padding: '4px 10px', borderRadius: 20, background: cfg.bg, color: cfg.color, border: `1.5px solid ${cfg.light}` }}>{c} {cnt}</span> : null;
            })}
          </div>
        </div>
  ...
  if (screen === 'training') return (
    <>
      <style>{gs}</style>
      <div style={page}>
        <button className="back-btn" onClick={() => setScreen('home')}><i className="ti ti-arrow-left"></i> ホームへ</button>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 20 }}>
          {CATEGORIES.map(c => {
            ...
            return <button key={c} onClick={() => setTrainingCat(c)} style={{ padding: '9px 16px', borderRadius: 20, cursor: 'pointer', fontSize: 14, fontWeight: 700, border: `2px solid ${active ? cfg.color : cfg.light}`, background: active ? cfg.color : cfg.bg, color: active ? '#fff' : cfg.color, transition: 'all 0.15s' }}>{c}</button>;
          })}
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
            {CATEGORIES.map(c => <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 700, color: CAT_CONFIG[c].color }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: CAT_CONFIG[c].color }}></div>{c}
            </div>)}
          </div>
        </div>
          ...
          <div style={{ ...card, borderTop: `4px solid ${CAT_CONFIG[trainingCat].color}` }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: CAT_CONFIG[trainingCat].bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className={`ti ${CAT_CONFIG[trainingCat].icon}`} style={{ fontSize: 18, color: CAT_CONFIG[trainingCat].color }}></i>
                </div>
              ...
              {getPastExercises(trainingCat).length > 0 && (
                <button onClick={() => setShowHistory(true)} style={{ padding: '7px 12px', borderRadius: 8, border: `1.5px solid ${CAT_CONFIG[trainingCat].color}`, background: CAT_CONFIG[trainingCat].bg, color: CAT_CONFIG[trainingCat].color, fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                  <i className="ti ti-history" style={{ fontSize: 14 }}></i> 履歴
                </button>
              )}
            </div>
            ...
            {exercise && sets && reps && (
              <div style={{ background: '#e8f4fd', borderRadius: 10, padding: '10px 14px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <p style={{ fontSize: 14, fontWeight: 700, color: '#1565c0' }}>推定消費: 約 {calcBurnKcal(trainingCat, parseInt(sets) || 1, parseInt(reps) || 0, weight)} kcal</p>
              </div>
            )}
        ...
        <div className="overlay" onClick={() => setShowHistory(false)}>
          <div className="sheet" onClick={e => e.stopPropagation()}>
            ...
            {getPastExercises(trainingCat).map(w => (
              <div key={w.exercise} className="hist-row" onClick={() => addFromHistory(w)}
                style={{ padding: '12px 14px', borderRadius: 10, border: '1.5px solid #eee', marginBottom: 8, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff' }}>
                <div>
                  <p style={{ fontWeight: 900, fontSize: 15, color: '#111', marginBottom: 2 }}>{w.exercise}</p>
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#888' }}>{w.sets}s × {w.reps}rep / {w.weight}kg</p>
                </div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <button onClick={e => { e.stopPropagation(); addWorkout(w.exercise, w.sets, w.reps, w.weight); }}
                    style={{ padding: '7px 14px', borderRadius: 8, border: 'none', background: CAT_CONFIG[trainingCat].color, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                    すぐ追加
                  </button>
                  <i className="ti ti-chevron-right" style={{ color: '#ccc', fontSize: 16 }}></i>
                </div>
              </div>
            ))}
          </div>
  ...
  if (screen === 'meal') return (
    <>
      <style>{gs}</style>
      <div style={page}>
        <button className="back-btn" onClick={() => { setShowCalendar(false); setScreen('home'); }}><i className="ti ti-arrow-left"></i> ホームへ</button>
        ...
        {!showCalendar ? (
          <>
            ...
            <button className="btn-outline" onClick={() => setShowCalendar(true)} style={{ width: '100%', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px' }}>
              <i className="ti ti-calendar"></i> カレンダーで確認
            </button>
              ...
              <div style={{ marginBottom: 12, position: 'relative' }}>
                ...
                {foodSugg.length > 0 && (
                  <div style={{ position: 'absolute', zIndex: 10, background: '#fff', border: '1.5px solid #ddd', borderRadius: 10, width: '100%', overflow: 'hidden', top: '100%', marginTop: 4, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                    {foodSugg.map(f => (
                      <div key={f} className="suggest-row" onClick={() => { setSelectedFood(f); setFoodQuery(f); setFoodSugg([]); }}
                        style={{ padding: '11px 14px', cursor: 'pointer', fontSize: 14, fontWeight: 700, borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', color: '#111' }}>
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              <button className="back-btn" style={{ marginBottom: 0 }} onClick={() => setShowCalendar(false)}><i className="ti ti-arrow-left"></i> 戻る</button>
            ...
            <div style={{ background: '#fff', border: '1.5px solid #e8e8e8', borderRadius: 16, padding: '14px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 3 }}>
                {getCalData(mealCalMonth, 'meal').map((cell, i) => {
                  ...
                  return (
                    <div key={i} style={{ borderRadius: 8, padding: '5px 2px', textAlign: 'center', minHeight: 72, background: hasData ? '#fafafa' : '#fafafa', border: '1px solid #eee' }}>
                      {cell && <>
